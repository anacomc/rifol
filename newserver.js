...
app.post('/generar-licencia-online', async (req, res) => {
    ...
    try {
        ...
        // A. INTERROGAMOS A TU TABLA DE DISPONIBLES EN SUPABASE
        const urlCheckStock = process.env.SUPABASE_DISPONIBLES + "?licencia=eq." + encodeURIComponent(lcLicenciaAValidar);
        const responseStock = await fetch(urlCheckStock, {
            method: 'GET',
            headers: { 'apikey': SUPABASE_KEY, 'Authorization': "Bearer " + SUPABASE_KEY, 'Content-Type': 'application/json' }
        ...
        if (!dataStock || dataStock.length === 0 || !Array.isArray(dataStock)) {
            return res.json({ exito: false, error: "Operación rechazada: El número de licencia no existe en el inventario." });
        }

        const registroStock = dataStock[0]; // Extraemos la fila de stock

        if (registroStock.status === true) {
            return res.json({ exito: false, error: "Operación rechazada: Esta licencia ya fue asignada previamente y consumió todos sus asientos." });
        }

        // Extraemos dinámicamente los topes de estaciones contratadas del lote
        const maxEstaciones = registroStock.limite_estaciones !== undefined ? parseInt(registroStock.limite_estaciones) : 1;
        ...
        const partesFecha = lcFin.trim().split('-');
        const lcVencimientoPostgres = partesFecha[2] + "-" + partesFecha[1] + "-" + partesFecha[0];
        ...
        const responseActi = await fetch(process.env.SUPABASE_ACTIVADAS, {
            ...
            body: JSON.stringify(payloadActivacionAsiento)
        });

        if (!responseActi.ok) {
            const txtErr = await responseActi.text();
            console.error("Supabase rechazó la inserción del asiento:", txtErr);
            return res.json({ exito: false, error: "El servidor de base de datos rechazó el alta del asiento." });
        }

        // =====================================================================
        // C. MOTOR CRIPTOGRÁFICO DE SELLADO ASIMÉTRICO RSA-SHA256 (MANTIENE TU XML)
        // =====================================================================
        const licenseData = "rif: " + lcRif.trim() + ";" + "nombre: " + lcNombre.trim() + ";" + "licencia: " + lcLicenciaAValidar + ";" + "expira: " + lcFin.trim() + ";";
        const dataBytes = Buffer.from(licenseData, 'utf8');
        const xmlPrivateKey = process.env.RSA_PRIVATE_KEY;
        ...
        console.log("✅ Licencia firmada y asiento inyectado en Supabase con éxito.");

        // Devolvemos el string unificado por el punto que tu FoxPro traga felizmente
        return res.status(200).json({
            exito: true,
            licenseFileContent: licenseFileContent
        });
        ...
        console.error("❌ Fallo crítico controlado en el soplete criptográfico:", error);
        return res.status(500).json({ exito: false, error: "Error en el servidor: " + error.message });
    }
});
